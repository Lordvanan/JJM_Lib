...
namespace JJM
{
	...
	private:
		...
		template <typename X>
		friend std::ostream& operator << (std::ostream& out, const List<X>& source);
	public:
		// Creates an empty data container with a size of zero.
		List() : size{ 0 }, iter{ nullptr } {
			data = new T[size];
		}

		// Creates a new data container using a defined initializer list.
		List(std::initializer_list<T> items) : size{ items.size() } {
			data = new T[size];
		...
		List(const List<T>& source) : size{ source.size } {
			data = new T[size];
			for (int i{ 0 }; i < size; i++)
	...
	template <typename T>
	std::ostream& operator << (std::ostream& out, const List<T>& source) {
		out << "[ ";
